...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateRecipeMutation = useMutation({
    ...
    onMutate: async ({ id, updates }) => {
      ...
      const previousRecipes = queryClient.getQueryData<Recipe[]>(['/api/recipes']);
      if (previousRecipes) {
        queryClient.setQueryData<Recipe[]>(['/api/recipes'], 
          previousRecipes.map(recipe => recipe.id === id ? { ...recipe, ...updates } : recipe)
        );
      }
    ...
    onSettled: () => queryClient.invalidateQueries({ queryKey: ['/api/recipes'] }),
  });
  ...
  const addIngredientsToGrocery = async (ingredients: string[], sourceMeal?: string) => {
    // Use server-side processing for proper parsing, filtering, and categorization
    await apiRequest('POST', '/api/grocery/add-ingredients', { ingredients, sourceMeal });
